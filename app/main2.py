        ...
        if st.button("Load from SharePoint"):
            ...
            if sharepoint_url:
                ...
                try:
                    ...
                    # ---------------- Convert Sharing Link to Server-Relative URL ----------------

                    # Example sharing link: https://.../:b:/t/AIChatBot/EfdiL8UNuIxGpirovP553bIBUN0dZucC8ylGQfS49wV6kw?e=XYZ

                    # Extract the base64 token if it's a :b: link

                    match = re.search(r":b:/t/[^/]+/([A-Za-z0-9_-]+)", sharepoint_url)

                    if match:

                        encoded = match.group(1)

                        decoded_bytes = base64.urlsafe_b64decode(encoded + "==")  # pad if necessary

                        server_relative_url = decoded_bytes.decode("utf-8")

                    else:

                        # fallback for normal direct URL

                        parsed = urlparse(sharepoint_url)

                        server_relative_url = unquote(parsed.path)
 
                    # Determine site collection URL

                    site_url_match = re.match(r"(https://[^/]+/sites/[^/]+)", server_relative_url)

                    if site_url_match:

                        site_url = site_url_match.group(1)

                        relative_url = server_relative_url.replace(site_url[len("https://devpwc365.sharepoint.com"):], "")

                    else:

                        # fallback: root site

                        site_url = "https://devpwc365.sharepoint.com"
