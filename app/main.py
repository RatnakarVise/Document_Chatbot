...
OPENAI_API_BASE = os.getenv("OPENAI_API_BASE")  # optional: custom base url (org proxy)
...
# Try to auto-load a default memory (optional)
AUTOLOAD = False
...
with left_col:
    ...
    if option == "Upload File":
        uploaded_file = st.file_uploader(
            "Upload PDF, DOCX, Excel, or ZIP (or upload a zipped folder)",
    ...
    # SHAREPOINT LINK
        ...
        if st.button("Load from SharePoint (Graph API)"):
            if not sharepoint_url:
                ...
                st.error("SharePoint credentials not set in .env (TENANT_ID, CLIENT_ID, CLIENT_SECRET).")
            elif not cache_name:
                st.warning("Please provide a unique cache name for this SharePoint load.")
            else:
                        ...
                        if "/:f:/" in sharepoint_url or "/:b:/" in sharepoint_url or "/:w:/" in sharepoint_url:
                            ...
                            if meta_json.get("folder"):
                                ...
                                if download_url:
                                    ...
                                    st.error("Could not get download URL from Graph metadata.")
                        else:
                            # regular path style: determine site and folder path
                            parsed = urlparse(sharepoint_url)
                            site_hostname = parsed.netloc
                            split = parsed.path.strip("/").split("/")
                            site_name = split[1] if len(split) > 1 else "sites"
    ...
    if st.button("Clear memory selection"):
        st.session_state.qa = None
        st.session_state.current_cache_name = None
        st.success("Cleared in-memory QA engine (the persisted cache remains on disk).")
...
with right_col:
    ...
    if query:
        if not st.session_state.qa:
            st.warning("No knowledge base loaded. Please upload or load from memory first.")
        else:
